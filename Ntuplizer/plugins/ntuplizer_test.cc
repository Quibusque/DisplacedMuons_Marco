...
void ntuplizer_test::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    std::cout << "Running on gen particles" << std::endl;
    Int_t nprugenmu = 0;
    ...
    std::cout << "Running on reco muons" << std::endl;
    auto start_all_recos = std::chrono::high_resolution_clock::now();
    // start reco muons loop
    ...
    std::cout << "Time taken to process reco muons: " << elapsed_all_recos.count() << " seconds"
              << std::endl;
    std::cout << "Average time per reco muon: " << elapsed_all_recos.count() / dmuons->size()
              << " seconds" << std::endl;
    // ------------------------------------------------
    // Gen Matching continued (OUTSIDE RECO MUONS LOOP)
    // ------------------------------------------------
    auto start_gen_matching = std::chrono::high_resolution_clock::now();
